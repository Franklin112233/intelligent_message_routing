...
- **Classification**: `app/classify.py` (interface); `app/mtl.py` (real MTL train + predict); stub in classify.py for fallback. Run/eval use MTL when `models/mtl_model.joblib` exists (train via `make train` or `python -m app.train_mtl`).
...
- **Docs**: README includes quick start (steps in order with commands), project layout table, commands table (all in Makefile), and system architecture (mermaid). Makefile default target is `help`; all run-related commands: install, train, run, test, eval.
